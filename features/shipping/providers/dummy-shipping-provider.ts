...
export class DummyShippingProvider implements ShippingProvider {
  ...
  async createShipment(input: CreateShipmentInput): Promise<CreateShipmentResult> {
    ...
    const providerShipmentId = `DSHP-${timestamp}-${randomSuffix}`;
    const trackingNumber = `RUPA-TRK-${timestamp}-${randomSuffix}`;

    const estDays = input.serviceCode.toUpperCase() === 'SAME_DAY' ? 0 : input.serviceCode.toUpperCase() === 'EXPRESS' ? 1 : 3;
  ...
  async getTracking(input: { trackingNumber: string }): Promise<TrackingResult> {
    ...
    return {
      ...
      provider: this.providerName,
      serviceName: 'RUPA Courier Express',
      status: 'IN_TRANSIT',
  ...
  async handleWebhook(
    ...
    const eventId = data.eventId || `dummy-ship-evt-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
...
export const dummyShippingProvider = new DummyShippingProvider();
